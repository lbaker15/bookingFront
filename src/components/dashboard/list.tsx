...
const List = React.memo(({ data, hasMore, dispatch, sortBy, filterBy }: EventListProps) => {
  const loading = () => {
    ...
    <div className='py-10'>
      <InfiniteScroll
        ...
        {data &&
          ...
          data
            .filter((item) => {
              return filterBy ? item.category.map((cat: string) => cat.toLowerCase()).includes(String(filterBy).toLowerCase()) : true;
            })
            .map((item, i) => {
              return <Event bookings={false} key={'event2' + i} item={item} />;
            })}
        {data &&
