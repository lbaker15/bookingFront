...
const Form = ({
	...
	const handleChange = (e: any) => {
		if (!setStateParent) {
			if (e.target.files) {
				...
				const newState = {
					...state,
					[e.target.name]: { value: null, valid: e.target.validity.valid },
				};
				setState(newState);
			} else {
				const newState = {
			...
			if (e.target.files) {
				...
				const newState = {
					...stateParent,
					[e.target.name]: { value: value, valid: e.target.validity.valid },
				};
	...
	return (
		<form data-testid="form" className="grid gap-8 md:grid-cols-2">
			...
			{secondBtn ? (
				<div className="w-full md:col-span-2 flex justify-between">
					<button
						className="mx-auto u-button"
						onClick={handleSubmit}
						type="submit"
					>
						{submitText ? submitText : "Submit"}
					</button>
					<Link to="/create" className="mx-auto u-button">
						{secondBtn}
					</Link>
				</div>
			) : (
